package week8.simulation.barbershop.vis;
...
import week8.simulation.barbershop.BarberState;
import week8.utils.Color;
...
public class BarberShopVisualizer implements SystemVisualizer {
    ...
    private void printTotalMoney() {
        printStat("Total Money",bs.getTotalMoney()+" TL",Color.YELLOW);
    }



    private void printCustomerCount() {
        printStat("Served Customer Count",bs.getTotalCustomerCount()+"",Color.WHITE);
    }

    private void printAverageWaitTime()
    {
        double averageWaitingTime = bs.getTotalCustomerCount()==0 ? 0: (double)bs.getTotalWaitTime()/(double)bs.getTotalCustomerCount();
        printStat("Average Waiting Time",String.format("%.1f",averageWaitingTime),Color.WHITE);
    }

    private void printRejectedCustomerCount() {
        printStat("Rejected Customer Count",bs.getRejectedCustomerCount()+"",Color.WHITE);
    }
    ...
    private void drawBarber(long clock, Barber b) {
        ...
        ConsolUtils.print(aligner.lAligned("SCC:" + b.getServedCustomerCount(), 10),Color.WHITE);
